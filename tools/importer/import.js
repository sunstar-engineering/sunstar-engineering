...
const createMetadata = (main, document, params) => {
  ...
  if (
    params.preProcessMetadata &&
    Object.keys(params.preProcessMetadata).length
  ) {
...
function addCarouselItems(document) {
  ...
  if (heroSlider) {
    ...
    const imageItemsFromDoc = document.querySelectorAll(
      ".swiper-slide:not(.swiper-slide-duplicate) .img-content"
    );
...
function removeCookiesBanner(document) {
  ...
  const cookieBanner = document.querySelector(
    ".cookies-wrapper.cookies-wrapper-js"
  );
  if (cookieBanner) {
    cookieBanner.remove();
  }
 ...
 */
function convertBackgroundImgsToForegroundImgs(
  sourceNode,
  targetNode = sourceNode
) {
  const bgImgs = sourceNode.querySelectorAll(
    ".background-image, .mobile-content"
  );
...
function createColumnBlockFromSection(document) {
  document.querySelectorAll("div.section-container").forEach((section) => {
    ...
    /* create a column block from the section
       ...
       * doesn't have lists
     */
    const heroParent = Array.from(section.parentElement.classList).filter((s) =>
      /hero/.test(s)
    ).length;
    ...
    const contentColumns = Array.from(section.children).filter(
      (el) =>
        el.tagName === "DIV" || el.tagName === "FIGURE" || el.tagName === "IMG"
    );
    if (
      !heroParent &&
      !hasEmbed &&
...
function createCardsBlockFromSection(document) {
  document.querySelectorAll("div.section-container").forEach((section) => {
    ...
    const sectionIsCard = section.parentElement.className.includes(
      "wp-block-sunstar-blocks-home-engineering-solution"
    );
    if (sectionIsCard) {
...
function createImgVariantsBlockFromSection(document) {
  document
    .querySelectorAll("div.section-container .mobile-content")
    .forEach((node) => {
...
function changeAnchorTags(document) {
  ...
  aTags.forEach((aTag) => {
    if (aTag.textContent.startsWith(basePath)) {
      aTag.textContent =
        homepage + (aTag.textContent === basePath ? "" : aTag.textContent);
    }
...
function fixRelativeLinks(document) {
  document.querySelectorAll("a").forEach((a) => {
    const targetDomain =
      "https://main--sunstar-engineering--sunstar-engineering.hlx.page";
...
export default {
      ...
      if (graphNode) {
        graphNode.forEach((node) => {
          ...
          if (
            ...
            const lastItem =
              node.itemListElement[node.itemListElement.length - 1];
  ...
  generateDocumentPath: ({
    ...
    if (
      pathname.toLowerCase().indexOf("/ja/news/") !== -1 &&
      preProcessMetadata &&
      preProcessMetadata.NewsDate
    ) {
      return WebImporter.FileUtils.sanitizePath(
        initialReplace.replace(
          /\/ja\/news\/.*$/,
          `/ja/news/${getFomattedDate(preProcessMetadata.NewsDate)}}`
        )
      );
    }
